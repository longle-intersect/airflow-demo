...
SCHEMA_NAME = "SDC_Sentinel_Collection_2"
...
# Define DAG
with DAG(
    "test-omd-sentinel2-full-lineage",
    default_args=default_args,
    description="Simulates Sentinel-2 satellite image processing pipeline with lineage",
    ...
    # Stage: fmaskcloud
    fmaskcloud_in = DummyOperator(
        task_id="fmaskcloud_input",
        inlets=[
            OMEntity(entity=Table, fqn=TABLES["ab0"], key="fmaskcloud"),
            OMEntity(entity=Table, fqn=TABLES["ab1"], key="fmaskcloud"),
            #OMEntity(entity=Table, fqn=TABLES["ab2"], key="fmaskcloud"),
        ],
    )
    fmaskcloud_out = DummyOperator(
        task_id="fmaskcloud_output",
        outlets=[
            OMEntity(entity=Table, fqn=TABLES["fmaskcloud_ad2"], key="fmaskcloud"),
            OMEntity(entity=Table, fqn=TABLES["fmaskcloud_ad3"], key="fmaskcloud"),
            OMEntity(entity=Table, fqn=TABLES["fmaskcloud_ad4"], key="fmaskcloud"),
        ],
    )

    # # Stage: topomasks
    topomasks_in = DummyOperator(
        task_id="topomasks_input",
        inlets=[
            OMEntity(entity=Table, fqn=TABLES["ab0"], key="topomask"),
        ],
    )
    topomasks_out = DummyOperator(
        task_id="topomasks_output",
        outlets=[
            OMEntity(entity=Table, fqn=TABLES["topomasks_ad0"], key="topomask"),
            OMEntity(entity=Table, fqn=TABLES["topomasks_ad1"], key="topomask"),
        ],
    )

    # # Stage: sfcref
    sfcref_in = DummyOperator(
        task_id="sfcref_input",
        inlets=[
            OMEntity(entity=Table, fqn=TABLES["ab0"], key="sfcref"),
            OMEntity(entity=Table, fqn=TABLES["ab1"], key="sfcref"),
        ],
    )
    sfcref_out = DummyOperator(
        task_id="sfcref_output",
        outlets=[
            OMEntity(entity=Table, fqn=TABLES["sfcref_aba"], key="sfcref"),
            OMEntity(entity=Table, fqn=TABLES["sfcref_abb"], key="sfcref"),
        ],
    )

    # # Stage: watermask
    watermask_in = DummyOperator(
        task_id="watermask_input",
        inlets=[
            OMEntity(entity=Table, fqn=TABLES["sfcref_aba"], key="watermask"),
            OMEntity(entity=Table, fqn=TABLES["sfcref_abb"], key="watermask"),
        ],
    )
    watermask_out = DummyOperator(
        task_id="watermask_output",
        outlets=[
            #OMEntity(entity=Table, fqn=TABLES["watermask_ad5"], key="watermask"),
            OMEntity(entity=Table, fqn=TABLES["watermask_ad6"], key="watermask"),
        ],
    )

    # # Stage: fractionalcover
    fractionalcover_in = DummyOperator(
        task_id="fractionalcover_input",
        inlets=[
            OMEntity(entity=Table, fqn=TABLES["sfcref_aba"], key="fractionalcover"),
            OMEntity(entity=Table, fqn=TABLES["sfcref_abb"], key="fractionalcover"),
        ],
    )
    fractionalcover_out = DummyOperator(
        task_id="fractionalcover_output",
        outlets=[
            OMEntity(entity=Table, fqn=TABLES["fractionalcover_ac0"], key="fractionalcover"),
        ],
    )

    # Define the processing sequence
    fmaskcloud_in >> fmaskcloud_out
    fmaskcloud_out >> topomasks_in
    topomasks_in >> topomasks_out
    topomasks_out >> sfcref_in
    sfcref_in >> sfcref_out
    sfcref_out >> watermask_in
    watermask_in >> watermask_out
    watermask_out >> fractionalcover_in
    fractionalcover_in >> fractionalcover_out