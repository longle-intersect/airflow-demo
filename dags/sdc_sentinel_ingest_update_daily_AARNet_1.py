...
def create_batch_tasks_callable(**context):
    ...
    #urls_list = urls.split()  # Adjust based on url_list format

    for idx, url in enumerate(urls_list[:2]):
        
        logger.info(f"URL List Split : {url}")
        ...
        # Add import_task here if needed
        download_task  # Trigger task creation
"""
...
def _download_url(url: Optional[str], aarnet_ssh_conn_id: str, shared_dir: str):
    """
    Download the content from a given URL via SSH using a curl command.
    Handles the case where no URL is provided.
    """
    if not url:
        logger.info("No URL to download for this task.")
        return None

    logger.info(f"Starting download function for URL: {url}")
    aarnet_hook = SSHHook(ssh_conn_id=aarnet_ssh_conn_id)

    try:
        with aarnet_hook as ssh_client:
            # Get final URI using HEAD request
            curl_cmd_filename = f"cd {shared_dir} && curl --silent -n --location --head {url}"
            stdin, stdout, stderr = ssh_client.exec_command(curl_cmd_filename)
            exit_status = stdout.channel.recv_exit_status()
            ...
            if exit_status != 0 or stderr_output or not uri:
                logger.error(f"Failed to resolve URI for {url}: {stderr_output}")
                return None  # Don't necessarily want to fail the whole DAG

            # Download file using the resolved URI
            curl_cmd = f"cd {shared_dir} && curl -n -L -O -J --silent --show-error {uri}"
            stdin, stdout, stderr = ssh_client.exec_command(curl_cmd)
            exit_status = stdout.channel.recv_exit_status()
            stderr_output = stderr.read().decode().strip()
            if exit_status != 0 or stderr_output:
        ...
        logger.error(f"Error during download for {url}: {str(e)}", exc_info=True)
        return None  # Don't necessarily want to fail the whole DAG
...
def daily_sentinel_batch_AARNet_processing_dag_1():
    ...
    # Define the TaskGroup outside of any callable
    with TaskGroup("download_tasks_group") as tg:
        pass  # This will be populated later

    def create_batch_tasks_callable_2(task_instance):
        urls = task_instance.xcom_pull(task_ids='search_new_images', key='url_list')
        if not urls:
            logger.info("No URLs to process.")
            return
    ...
    create_batch_tasks = PythonOperator(
        task_id='create_batch_tasks',
        python_callable=create_batch_tasks_callable_2
    )
